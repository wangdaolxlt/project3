...
import com.mall.order.dto.OrderDetailRespVo;
import com.mall.order.dto.UpdateOrderRequest;
import com.mall.order.dto.UpdateOrderResponse;
...
import com.mall.pay.service.config.Configs;
import com.mall.pay.service.model.ExtendParams;
import com.mall.pay.service.model.GoodsDetail;
import com.mall.pay.service.model.builder.*;
import com.mall.pay.service.model.hb.*;
import com.mall.pay.service.model.result.AlipayF2FPayResult;
import com.mall.pay.service.model.result.AlipayF2FPrecreateResult;
import com.mall.pay.service.model.result.AlipayF2FQueryResult;
import com.mall.pay.service.model.result.AlipayF2FRefundResult;
import com.mall.pay.service.service.AlipayMonitorService;
import com.mall.pay.service.service.AlipayTradeService;
import com.mall.pay.service.service.impl.AlipayMonitorServiceImpl;
import com.mall.pay.service.service.impl.AlipayTradeServiceImpl;
import com.mall.pay.service.service.impl.AlipayTradeWithHBServiceImpl;
import com.mall.pay.service.utils.Utils;
import com.mall.pay.service.utils.ZxingUtils;
...
/**
 * @PackgeName: com.mall.pay.service
...
/*    public static void main(String[] args) {
        ...
        // 测试查询当面付2.0交易
        //payService.test_trade_query();

        // 测试当面付2.0退货
        //        main.test_trade_refund();

        // 测试当面付2.0生成支付二维码
        //payService.test_trade_precreate();
    }*/
    ...
    @Override
    @Transactional
    public PaymentResponse aliPay(PaymentRequest request) {
        PaymentResponse response = new PaymentResponse();
        /**
         * 前端传入金额有问题，先自己查一下订单金额
         *
         */
        OrderDetailRequest orderDetailRequest = new OrderDetailRequest();
        orderDetailRequest.setOrderId(request.getOrderId());
        OrderDetailRespVo orderDetailRespVo = orderQueryService.orderDetail(orderDetailRequest);
        BigDecimal orderTotal = orderDetailRespVo.getOrderTotal();
        //插入payment表
        Payment payment = new Payment();
        payment.setStatus("0");
        payment.setOrderId(request.getOrderId());
        payment.setProductName(request.getProductName());
        payment.setPayerUid(request.getPayerUid());
        payment.setPayerName(request.getPayerName());
        payment.setOrderAmount(orderTotal);
        payment.setPayerAmount(orderTotal);
        payment.setPayWay(request.getPayWay());
        payment.setRemark(request.getRemark());
        Date date = new Date();
        payment.setCreateTime(date);
        payment.setUpdateTime(date);
        int insertCode = paymentMapper.insert(payment);
        if(insertCode < 1){
            response.setCode(PayRetCode.DB_EXCEPTION.getCode());
            response.setMsg(PayRetCode.DB_EXCEPTION.getMessage());
            return response;
        }

        //生成二维码
        CreateQRCodeRequest createQRCodeRequest = new CreateQRCodeRequest();
        createQRCodeRequest.setOrderId(request.getOrderId());
        createQRCodeRequest.setTotalAmount(orderTotal.toString());
        CreateQRCodeResponse createQRCodeResponse = test_trade_precreate(createQRCodeRequest);
        response.setCode(createQRCodeResponse.getCode());
        response.setMsg(createQRCodeResponse.getMsg());
        response.setPicName(createQRCodeResponse.getQRCode());
        return response;
    }
     ...
     */
    @Override
    public QueryAlipayStatusResponse queryAlipayStatus(PaymentRequest request) {
        String orderId = request.getOrderId();
        //如果是没支付就直接返回
        QueryAlipayStatusResponse response = test_trade_query(orderId);
        ...
        UpdateOrderResponse updateOrderResponse = orderCoreService.updateOrder(updateOrderRequest);
        if(! updateOrderResponse.getCode().equals(OrderRetCode.SUCCESS.getCode())){
            response.setCode(updateOrderResponse.getCode());
            response.setMsg(updateOrderResponse.getMsg());
            return response;
        }
        ...
        int updateRow = paymentMapper.updateByPrimaryKeySelective(payment);
        if(updateRow < 1){
            response.setCode(PayRetCode.DB_EXCEPTION.getCode());
            response.setMsg(PayRetCode.DB_EXCEPTION.getMessage());
            return response;
    ...
    // 测试当面付2.0查询订单
    public QueryAlipayStatusResponse test_trade_query(String orderId) {
        ...
        QueryAlipayStatusResponse statusResponse = new QueryAlipayStatusResponse();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                statusResponse.setCode(PayRetCode.SUCCESS.getCode());
                return statusResponse;

            case FAILED:
                log.error("查询返回该订单支付失败或被关闭!!!");
                statusResponse.setCode(PayRetCode.ORDER_FAILED.getCode());
                statusResponse.setMsg(PayRetCode.ORDER_FAILED.getMessage());
                return statusResponse;
            case UNKNOWN:
                log.error("系统异常，订单支付状态未知!!!");
                statusResponse.setCode(PayRetCode.ORDER_UNKNOWN.getCode());
                statusResponse.setMsg(PayRetCode.ORDER_UNKNOWN.getMessage());
                return statusResponse;

            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                statusResponse.setCode(PayRetCode.ORDER_NON_SUPPORT.getCode());
                statusResponse.setMsg(PayRetCode.ORDER_NON_SUPPORT.getMessage());
                return statusResponse;
        }
    }
     ...
     * @return
     */
    public CreateQRCodeResponse test_trade_precreate(CreateQRCodeRequest request) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = request.getOrderId();

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "csmall小摊消费";
        ...
        String body = "购买商品共"+ request.getTotalAmount() +"元";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("D:\\WangDao\\Project3\\picture\\qr-%s.png",
                        response.getOutTradeNo());
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                createQRCodeResponse.setCode(PayRetCode.SUCCESS.getCode());
                createQRCodeResponse.setMsg(PayRetCode.SUCCESS.getMessage());
                createQRCodeResponse.setQRCode("qr-"+outTradeNo+".png");
                return createQRCodeResponse;


            case FAILED:
                log.error("支付宝预下单失败!!!");
                createQRCodeResponse.setCode(PayRetCode.ORDER_FAILED.getCode());
                createQRCodeResponse.setMsg(PayRetCode.ORDER_FAILED.getMessage());
                return createQRCodeResponse;

            case UNKNOWN:
                log.error("系统异常，预下单状态未知!!!");
                createQRCodeResponse.setCode(PayRetCode.ORDER_UNKNOWN.getCode());
                createQRCodeResponse.setMsg(PayRetCode.ORDER_UNKNOWN.getMessage());
                return createQRCodeResponse;
            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                createQRCodeResponse.setCode(PayRetCode.ORDER_NON_SUPPORT.getCode());
                createQRCodeResponse.setMsg(PayRetCode.ORDER_NON_SUPPORT.getMessage());
                return createQRCodeResponse;
        }
    }
